...
const startup = async () => {
	try {
		...
		for (const job of allJobs) {
			try {
				...
				if (job.runCountCompleted >= job.runCount) {
					// If the job has completed all its runs, update its status to inactive and stop the job
					await MontySms.findByIdAndUpdate(job._id, { status: 'Inactive' });
					events.emit('taskUpdated', { taskId: job._id, status: 'Inactive' });
					continue; // Skip scheduling for completed jobs
				}
		...
		console.error('Error fetching jobs on startup:', err);
	}
};

// Check and update the status of all tasks if the tasks are active
