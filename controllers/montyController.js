...
const sendNow = async (req, res) => {
	try {
		...
		for (const phoneNumber of phoneNumbers) {
			if (!/^234\d{10}$/.test(phoneNumber)) {
				return res.status(400).json({
					error: 'Enter valid Nigerian numbers, separated by commas. Numbers should start with 234 and be 13 digits long.',
				});
			}
		}
		...
		try {
			// Try to send the SMS
			await sendSMS(message, title);
			return res.status(200).json({ message: `SMS delivered successfully at ${time}` });
		} catch (error) {
			return res.status(500).json({ error: error.message });
		}
	} catch (err) {
		console.error('Error Sending SMS:', err);
		return res.status(500).json({ error: 'There was an error sending the SMS. Please try again later.' });
	}
};
